...
def fetch_game_data(platform_id, access_token, max_rating):
    ...
    payload = f"""
    fields name, rating;
    where platforms = {platform_id} & rating < {max_rating};
    sort rating desc;
    limit 500;
    """
...
# Fetch Nintendo DS games with a rating below the specified minimum rating
nintendo_ds_games = fetch_game_data(platform_id, access_token, max_rating)

# Store game data in a dictionary
games = {}
for game in nintendo_ds_games:
    games[game['name']] = game['rating']

# List for storing matched file names
matched_files = []

# Iterate through your local game files
for file_name in os.listdir(game_files_path):
    cleaned_name = clean_game_name(file_name)
    if cleaned_name in games:
        matched_files.append(file_name)

# Print a summary of the matched files
if len(matched_files) > 0:
    print(f"\n{len(matched_files)} files matched the specified criteria.")
    ...
    print("No files matched the specified criteria. No files will be removed.")